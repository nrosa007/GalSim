...
def func(file_name, random_seed, pixel_scale, nx, ny, sky_level, gal_flux_min, gal_flux_max, gal_hlr_min, gal_hlr_max, gal_e_min, gal_e_max, psf_fwhm, gsparams, psf1, psf2, psf3_inner, psf3_outer, psf3, atmos, aberrations, psf4, optics,  psf5, psfs, psf_names, psf_times, psf_fft_times, psf_phot_times, gal1, gal2, gal3, gal4, bulge, disk, gal5, gals, gal_names, gal_times, gal_fft_times, gal_phot_times, setup_times, fft_times, phot_times, noise_times, k, all_fluxes_i, psf, psf_name, gal, gal_name):
    ...
    hlr = (gal_hlr_max-gal_hlr_min) + gal_hlr_min
    ...
    return image, t1, t2, t3, t4, t5, t6, k, flux, hlr, gal_shape, psfs, gals, file_name

    
def main(num_sersic_n, num_HLR, num_FWHM, num_nx_ny, num_folding_threshold, num_maxk_threshold, num_xvalue_accuracy, num_kvalue_accuracy, num_shoot_accuracy, num_pixel_scale):
    
    write_file = open('output.tx','w')
    ...
    for c in num_sersic_n:
        ...
        for a in num_HLR:
            ...
            for b in num_FWHM:
                ...
                for d in num_nx_ny:
                    o4 = []
                    for e in num_folding_threshold:
                        o5 = []
                        for f in num_maxk_threshold:
                            o6 = []
                            for g in num_xvalue_accuracy:
                                o7 = []
                                for h in num_kvalue_accuracy:
                                    o8 = []
                                    for i in num_shoot_accuracy:
                                        o9= []
                                        for j in num_pixel_scale:
                                            random_seed = 553728
                                            sky_level = 1.e4        # ADU / arcsec^2
                                            pixel_scale = j     # arcsec
                                            nx = d
                                            ny = d

                                            gal_flux_min = 100     # Range for galaxy flux
                                            gal_flux_max = 1.e5
                                        ...
                                        # documentation, including the default values for each parameter.
                                            gsparams = galsim.GSParams(
                                                folding_threshold=e, # maximum fractional flux that may be folded around edge of FFT
                                                maxk_threshold = f,    # k-values less than this may be excluded off edge of FFT
                                                xvalue_accuracy = g,   # approximations in real space aim to be this accurate
                                                kvalue_accuracy = h,   # approximations in fourier space aim to be this accurate
                                                shoot_accuracy = i,    # approximations in photon shooting aim to be this accurate
                                                minimum_fft_size=50)     # minimum size of ffts

                                            logger.info('Starting demo script 7')

                                        # Make the PSF profiles:
                                            psf1 = galsim.Gaussian(fwhm = b, gsparams=gsparams)
                                            psf2 = galsim.Moffat(fwhm = b, beta = 2.4, gsparams=gsparams)
                                            psf3_inner = galsim.Gaussian(fwhm = b, flux = 0.8, gsparams=gsparams)
                                            psf3_outer = galsim.Gaussian(fwhm = b, flux = 0.2, gsparams=gsparams)
                                            psf3 = psf3_inner + psf3_outer
                                            atmos = galsim.Gaussian(fwhm = b, gsparams=gsparams)
                                        ...
                                        #aberrations = [0.0, 0.0, 0.0, 0.0, 0.06, 0.12, -0.08, 0.07, 0.04, 0.0, 0.0, -0.13]
                                            ...
                                            atmos = galsim.Kolmogorov(fwhm = b, gsparams=gsparams)
                                            ...
                                            psf_times = [0]*len(psfs)
                                            psf_fft_times = [0,0,0,0,0]
                                            psf_phot_times = [0,0,0,0,0]

                                        # Make the galaxy profiles:
                                            gal1 = galsim.Gaussian(half_light_radius = a, gsparams=gsparams)
                                            gal2 = galsim.Exponential(half_light_radius = a, gsparams=gsparams)
                                            gal3 = galsim.DeVaucouleurs(half_light_radius = a, gsparams=gsparams)
                                            gal4 = galsim.Sersic(half_light_radius = a, n = c , gsparams=gsparams)
                                        ...
                                        # you are using for all the size values as defined by the pixel_scale).
                                            bulge = galsim.Sersic(flux=0.4, half_light_radius=a, n=3.2, trunc=8.5, gsparams=gsparams)
                                            disk = galsim.Sersic(flux=0.6, half_light_radius=a, n=1.5, gsparams=gsparams)
                                        ...
                                        # Loop over combinations of psf, gal, and make 4 random choices for flux, size, shape.
                                            ...
                                            # it is omitted from the str for brevity.
                                                ...
                                                for igal in range(len(gals)):
                                                    ...
                                                    ### Stuff below here has been changed by Melanie!
                                                    
                                                    all_fluxes_i = gal_flux_min
                                                    image, t1, t2, t3, t4, t5, t6, k, flux, hlr, gal_shape, psfs, gals, file_name = func(file_name, random_seed, pixel_scale, nx, ny, sky_level, gal_flux_min, gal_flux_max, gal_hlr_min, gal_hlr_max, gal_e_min, gal_e_max, psf_fwhm, gsparams, psf1, psf2, psf3_inner, psf3_outer, psf3, atmos, aberrations, psf4, optics,  psf5, psfs, psf_names, psf_times, psf_fft_times, psf_phot_times, gal1, gal2, gal3, gal4, bulge, disk, gal5, gals, gal_names, gal_times, gal_fft_times, gal_phot_times, setup_times, fft_times, phot_times, noise_times, k, all_fluxes_i, psf, psf_name, gal, gal_name)
                                                    ...
                                                    # Keep track of the low and high points
                                                    flux_low = all_fluxes_i
                                                    time_low = (t5-t4) - (t3-t2) # This is photon-shooting time minus FFT time: should be + at high F, - at low F
                                                    
                                                    # Copying this again, only now doing the high flux point
                                                    all_fluxes_i = gal_flux_max
                                                    image, t1, t2, t3, t4, t5, t6, k, flux, hlr, gal_shape, psfs, gals, file_name = func(file_name, random_seed, pixel_scale, nx, ny, sky_level, gal_flux_min, gal_flux_max, gal_hlr_min, gal_hlr_max, gal_e_min, gal_e_max, psf_fwhm, gsparams, psf1, psf2, psf3_inner, psf3_outer, psf3, atmos, aberrations, psf4, optics,  psf5, psfs, psf_names, psf_times, psf_fft_times, psf_phot_times, gal1, gal2, gal3, gal4, bulge, disk, gal5, gals, gal_names, gal_times, gal_fft_times, gal_phot_times, setup_times, fft_times, phot_times, noise_times, k, all_fluxes_i, psf, psf_name, gal, gal_name)
                                                    ...
                                                    # Keep track of the low and high points
                                                    flux_high = all_fluxes_i
                                                    time_high = (t5-t4) - (t3-t2) # This is photon-shooting time minus FFT time: should be + at high F, - at low F
                                                    crossover_flux = []
                                                    if np.sign(time_high)==np.sign(time_low):
                                                        logger.info("Same sign--no crossover flux!")
                                                        _flux = 0
                                                        crossover_flux = np.append(crossover_flux, _flux)
                                                        logger.info("Crossover flux is %f"%_flux)
                                                    else:
                                                        while flux_high/flux_low > 1.05: # 1% flux resolution is PROBABLY fine, but we should check
                                                            all_fluxes_i = np.sqrt(flux_high*flux_low) # Geometric mean, better for log-scaled variables
                                                            logger.info("Trying flux %f"%all_fluxes_i)
                                                            image, t1, t2, t3, t4, t5, t6, k, flux, hlr, gal_shape, psfs, gals, file_name = func(file_name, random_seed, pixel_scale, nx, ny, sky_level, gal_flux_min, gal_flux_max, gal_hlr_min, gal_hlr_max, gal_e_min, gal_e_max, psf_fwhm, gsparams, psf1, psf2, psf3_inner, psf3_outer, psf3, atmos, aberrations, psf4, optics,  psf5, psfs, psf_names, psf_times, psf_fft_times, psf_phot_times, gal1, gal2, gal3, gal4, bulge, disk, gal5, gals, gal_names, gal_times, gal_fft_times, gal_phot_times, setup_times, fft_times, phot_times, noise_times, k, all_fluxes_i, psf, psf_name, gal, gal_name)
                                                            ...
                                                            # Keep track of the low and high points
                                                            this_time = (t5-t4) - (t3-t2)
                                                            if np.sign(time_high)==np.sign(this_time):
                                                                flux_high = all_fluxes_i
                                                                time_high = this_time
                                                            else:
                                                                flux_low = all_fluxes_i
                                                                time_low = this_time
                                                        _flux = np.sqrt(flux_high*flux_low)
                                                        crossover_flux = np.append(crossover_flux, _flux)
                                                        logger.info("Crossover flux is %f"%_flux)
                                                    pix_val = np.append(pix_val, pixel_scale)
                                                    shoot_val = np.append(shoot_val, e)
                                                    kvalue_val = np.append(kvalue_val, f)
                                                    xvalue_val = np.append(xvalue_val, g)
                                                    maxk_val = np.append(maxk_val, h)
                                                    folding_val = np.append(folding_val, i)
                                                    nxny_val = np.append(nxny_val, nx)
                                                    FWHM_val = np.append(FWHM_val, fwhm)
                                                    HLR_val = np.append(HLR_val, half_light_radius)
                                                    sersic_val = np.append(sersic_val, n)
                                                    gal_val = np.append(gal_val,gal_names[igal])
                                                    psf_val = np.append(psf_val, psf_names[ipsf])
                                                    crossover = np.append(crossover, crossover_flux)
                                                pix_values = np.append(pix_values, pix_val)
                                                shoot_values = np.append(shoot_values, shoot_val)
                                                kvalue_values = np.append(kvalue_values, kvalue_val)
                                                xvalue_values = np.append(xvalue_values, xvalue_val)
                                                maxk_values = np.append(maxk_values, maxk_val)
                                                folding_values = np.append(folding_values, folding_val)
                                                nxny_values = np.append(nxny_values, nxny_val)
                                                FWHM_values = np.append(FWHM_values, FWHM_val)
                                                HLR_values = np.append(HLR_values, HLR_val)
                                                sersic_values = np.append(sersic_values, sersic_val)
                                                gal_values = np.append(gal_values, gal_val)
                                                psf_values = np.append(psf_values, psf_val)
                                                cross = np.append(cross, crossover)
                                            pix_output = np.append(pix_output, pix_values)
                                            shoot_output = np.append(shoot_output, shoot_values)
                                            kvalue_output = np.append(kvalue_output, kvalue_values)
                                            xvalue_output = np.append(xvalue_output, xvalue_values)
                                            maxk_output = np.append(maxk_output, maxk_values)
                                            folding_output = np.append(folding_output, folding_values)
                                            nxny_output = np.append(nxny_output, nxny_values)
                                            FWHM_output = np.append(FWHM_output, FWHM_values)
                                            HLR_output = np.append(HLR_output, HLR_values)
                                            sersic_output = np.append(sersic_output, sersic_values)
                                            GAL_output = np.append(GAL_output, gal_values)
                                            PSF_output = np.append(PSF_output, psf_values)
                                            o9 = np.append(o9, cross)
                                        o8 = np.append(o8, o9)
                                    o7 = np.append(o7, o8)
                                o6 = np.append(o6, o7)
                            o5 = np.append(o5, o6)
                ...
                o2 = np.append(o2, o3)
            o1 = np.append(o1, o2)
        over = np.append(over, o1)
    return over, PSF_output, GAL_output, pix_output, shoot_output, kvalue_output, xvalue_output, maxk_output, folding_output, nxny_output, FWHM_ouput, HLR_output, sersic_output
